...
#ifdef S4_EIGEN_BACKEND_MEKIL
	int rank = 0;
	mekil::lapack::linear_least_squares<double>(
		static_cast<int>(m), static_cast<int>(n), static_cast<int>(nRHS),
		a, static_cast<int>(lda), b, static_cast<int>(ldb), rcond, &rank);
...
static int Invert(size_t n, std::complex<double> *a, size_t lda, std::complex<double> *work, size_t lwork, size_t *iwork){
	// Match prior zgetrf_+zgetri_: info>0 (singular U) still inverts factors; only info from getri is returned.
	mekil::lapack::lu_factor<double>(n, n, a, lda, iwork);
	return mekil::lapack::lu_invert<double>(n, a, lda, iwork, work, lwork);
}
...
static void Mult(size_t n, const double &alpha, const std::complex<double> *a, size_t lda, std::complex<double> *b, size_t ldb, const double &beta, std::complex<double> *c, size_t ldc){
	mekil::lapack::gemm<double>('N', 'N', n, n, n, std::complex<double>(alpha), a, lda, b, ldb, std::complex<double>(beta), c, ldc);
}
static void Mult(size_t n, size_t m, const double &alpha, const std::complex<double> *a, size_t lda, std::complex<double> *b, const double &beta, std::complex<double> *c){
	mekil::lapack::gemv<double>('N', n, m, std::complex<double>(alpha), a, lda, b, 1, std::complex<double>(beta), c, 1);
}
static int LUFactor(size_t n, std::complex<double> *a, size_t lda, size_t *ipiv){
	return mekil::lapack::lu_factor<double>(n, n, a, lda, ipiv);
}
static int LUSolve(size_t n, size_t nRHS, const std::complex<double> *a, size_t lda, const size_t *ipiv, std::complex<double> *b, size_t ldb){
	return mekil::lapack::lu_solve<double>('N', n, nRHS, a, lda, ipiv, b, ldb);
}
#else
