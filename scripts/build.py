#!/usr/bin/env python3
"""Build standalone S4lua (RNP+MKL) into --build-dir. Does not configure simulation_core."""
...
def find_s4lua(build_dir: Path, platform: str) -> Path:
    ...
    for c in (build_dir / name, build_dir / "Release" / name, build_dir / "RelWithDebInfo" / name):
        if c.is_file():
            return c
    _die(f"build finished but missing {name} under {build_dir} (checked ., Release/, RelWithDebInfo/)")
...
def _build_win(build_dir: Path, *, skip_smoke: bool) -> Path:
    # Ninja + cl after vcvars (matches core Windows builds).
    cmake = [
        "cmake",
        "-S",
        str(S4_ROOT),
    ...
    if not skip_smoke:
        _smoke(binary, os.environ.copy())
    return binary


def _smoke(binary: Path, env: dict[str, str]) -> None:
